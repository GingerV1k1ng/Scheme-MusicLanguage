...
; Kurt Normark, 2017.
; Relies on pp-standard-functions and pp-standard-higher-order-functions, as provided for the PP course.
...
(define (note-abs-time-with-duration abs-time channel note-number velocity duration)
  (if (valid-input-to-note-abs-time-with-duration? abs-time channel note-number velocity duration)
      (tag 'note-abs-time-with-duration (list abs-time channel note-number velocity duration))
...
;                                       (lambda (x y) (<= (low-level-note-abs-time x) (low-level-note-abs-time y))))
         ...
         (end-of-track-events (map convert-event-to-byte-string (list (midi-meta 0 47 ""))))  ; a list of byte string
...
(define (midi-noteon abs-time channel note-number velocity)
  (tag 'noteon (list abs-time channel note-number velocity)))

(define (midi-noteoff abs-time channel note-number velocity)
  (tag 'noteoff (list abs-time channel note-number velocity)))

(define (midi-programchange abs-time channel program-number)
  (tag 'programchange (list abs-time channel program-number)))

(define (midi-meta abs-time type contents)
  (tag 'meta (list abs-time type contents)))
...
; Functions that handle byte strings as strings of single byte chars.
(define byte-string-append string-append)
(define byte-string-length string-length)
(define byte-string-ref string-ref)
...
(define (int10-to-binary n number-of-bytes)
  (let* ((byte-list (binary-bytes-of-decimal-integer n))
    ...
    (if (> lgt-byte-list number-of-bytes)
        ...
        (list->string (append
                         (make-list (- number-of-bytes lgt-byte-list) (integer->char 0))    ; pad with initial zeros
                         byte-list)))))
...
(define (binary-bytes-of-decimal-integer-1 n)
 (let ((rem (remainder n 256))
       (rest (quotient n 256)))
  (if (= rest 0)
      (list (integer->char rem))
      (cons (integer->char rem) (binary-bytes-of-decimal-integer-1 rest)))))
...
(define (make-char-2-hex hx1 hx2 )
  (integer->char (+ (* hx1 16) hx2)))  ; was as-char...
...
(define (unparse-integer-as-variable-length-quantity n)
  (let ((low (remainder n 128))
        (rest (quotient n 128)))
    (byte-string-append (unparse-integer-as-variable-length-quantity-highs rest) (as-string (as-char low)))))
...
(define (unparse-integer-as-variable-length-quantity-highs n)
  (if (= n 0)
      ...
      (let ((low (remainder n 128))
            (rest (quotient n 128)))
        (byte-string-append (unparse-integer-as-variable-length-quantity-highs rest) (as-string (as-char (+ 128 low)))) ;    + 128  enforces one as first bit.
        )))
...
(define (as-string x)
  (cond ((number? x) (number->string x))
        ...
        ((pair? x)
            (byte-string-append "(" 
               (apply byte-string-append
                  (map (lambda (y) (byte-string-append (as-string y) " ")) (proper-part x))
               )
